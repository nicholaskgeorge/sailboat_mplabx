...
#ifndef ANEMOMETER_H    /* Guard against multiple inclusion */
#define ANEMOMETER_H
...
void realtive_to_earth(Anemometer_INFO* aminfo, IMU_INFO* imuinfo, int mast_angle){
    ...
    double imu_phi = imuinfo->pitch; //raw pitch angle
    //First Correction:
    ...
    double cor1_phi = imu_alpha; //raw pitch angle
    //Second Correction:
    ...
    double cor2_phi = -1*cor1_phi; //raw pitch angle
